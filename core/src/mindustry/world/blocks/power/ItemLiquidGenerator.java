...
public class ItemLiquidGenerator extends PowerGenerator{
    ...
    protected void setDefaults(){
        if(hasItems){
            consumes[0].add(new ConsumeItemFilter(item -> getItemEfficiency(item) >= minItemEfficiency)).update(false).optional(true, false);
        }

        if(hasLiquids){
            consumes[0].add(new ConsumeLiquidFilter(liquid -> getLiquidEfficiency(liquid) >= minLiquidEfficiency, maxLiquidGenerate)).update(false).optional(true, false);
        }
