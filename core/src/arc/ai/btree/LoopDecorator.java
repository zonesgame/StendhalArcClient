...
/** {@code LoopDecorator}是一个抽象的类, 为循环执行装饰器提供基本功能.<p/>
...
public abstract class LoopDecorator<E> extends Decorator<E> {
	...
	/** {@link #run()}是否必须保持循环.<p/>Whether the {@link #run()} method must keep looping or not. */
	protected boolean loop;

	/** 创建一个没有子任务的循环包装器.<p/>Creates a loop decorator with no child task. */
