...
public class BehaviorTree<E> extends Task<E> {
	...
	/** 行为树绑定对象.<p/>Returns the blackboard object of this behavior tree. */
	@Override
	public E getObject () {
		return object;
	}
	...
	public void removeListener (Listener<E> listener) {
		if (listeners != null) listeners.remove(listener, true);
	}
	...
	private static final class GuardEvaluator<E> extends Task<E> {
		...
		// 无参构造函数用于Kryo序列化.<p/>No argument constructor useful for Kryo serialization
	...
	public interface Listener<E> {
		...
		/** 在设置任务状态时调用该方法. 这并不一定意味着状态已经改变.<p/>
		...
		/**  当将一个子任务添加到父任务的children时, 调用该方法.<p/>This method is invoked when a child task is added to the children of a parent task.
