...
import arc.ai.btree.annotation.TaskAttribute;
import arc.ai.btree.annotation.TaskConstraint;
import arc.ai.btree.branch.DynamicGuardSelector;
import arc.ai.btree.branch.Parallel;
import arc.ai.btree.branch.RandomSelector;
import arc.ai.btree.branch.RandomSequence;
import arc.ai.btree.branch.Selector;
import arc.ai.btree.branch.Sequence;
import arc.ai.btree.decorator.AlwaysFail;
import arc.ai.btree.decorator.AlwaysSucceed;
import arc.ai.btree.decorator.Include;
import arc.ai.btree.decorator.Invert;
import arc.ai.btree.decorator.Random;
import arc.ai.btree.decorator.Repeat;
import arc.ai.btree.decorator.SemaphoreGuard;
import arc.ai.btree.decorator.UntilFail;
import arc.ai.btree.decorator.UntilSuccess;
import arc.ai.btree.leaf.Failure;
import arc.ai.btree.leaf.Success;
import arc.ai.btree.leaf.Wait;
import arc.ai.utils.random.Distribution;
import arc.files.Fi;
...
import arc.util.reflect.Annotation;
...
public class BehaviorTreeParser<E> {
	...
	public BehaviorTree<E> parse (Fi file, E object) {
		btReader.parse(file);
		return createBehaviorTree(btReader.root, object);
	}
	...
	public static class DefaultBehaviorTreeReader<E> extends BehaviorTreeReader {
		...
		enum Statement {
			...
			Subtree("subtree") {
				...
				protected <E> boolean attribute (DefaultBehaviorTreeReader<E> reader, String name, Object value) {
					...
					if (!(value instanceof String)) reader.throwAttributeTypeException(this.name, name, "String");
					if ("".equals(value)) throw new ArcRuntimeException(this.name + ": the name connot be empty");
					if (reader.subtreeName != null)
						throw new ArcRuntimeException(this.name + ": the name has been already specified");
				...
				protected <E> void exit (DefaultBehaviorTreeReader<E> reader) {
					if (reader.subtreeName == null)
						throw new ArcRuntimeException(this.name + ": the name has not been specified");
		...
		public void parse (char[] data, int offset, int length) {
			...
			if (rootTree == null) throw new ArcRuntimeException("Missing root tree");
			root = rootTree.rootTask;
			if (root == null) throw new ArcRuntimeException("The tree must have at least the root task");
		...
		protected void startStatement (String name, boolean isSubtreeReference, boolean isGuard) {
			...
			if (isGuard) {
				if (statement != Statement.TreeTask)
					throw new ArcRuntimeException(name + ": only tree's tasks can be guarded");
			}
		...
		protected void attribute (String name, Object value) {
			...
			if (!validAttribute) {
				if (statement == Statement.TreeTask) {
					...
					throw new ArcRuntimeException(statement.name + ": unknown attribute '" + name + "'");
				}
			}
		}
		...
		protected Object castValue (Field field, Object value) {
			...
			if (value instanceof Number) {
				...
				if (type == String.class)
					...
					if (stringValue.length() != 1) throw new ArcRuntimeException("Invalid character '" + value + "'");
		...
		private void throwAttributeNameException (String statement, String name, String expectedName) {
			...
			throw new ArcRuntimeException(statement + ": attribute '" + name + "' unknown; " + expected);
		}

		private void throwAttributeTypeException (String statement, String name, String expectedType) {
			throw new ArcRuntimeException(statement + ": attribute '" + name + "' must be of type " + expectedType);
		}
		...
		private void openTask (String name, boolean isGuard) {
			try {
				...
				throw new ArcRuntimeException("Cannot parse behavior tree!!!", e);
			}
		}
		...
		private StackedTask<E> createStackedTask (String name, Task<E> task) {
			...
			if (metadata == null)
				throw new ArcRuntimeException(name + ": @TaskConstraint annotation not found in '" + task.getClass().getSimpleName()
					+ "' class hierarchy");
		...
		void switchToNewTree(String name) {
			...
			if (oldTree != null)
				throw new ArcRuntimeException("A subtree named '" + name + "' is already defined");
		}
		...
		Task<E> subtreeRootTaskInstance(String name) {
			...
			if (tree == null)
				throw new ArcRuntimeException("Undefined subtree with name '" + name + "'");
		...
		void addImport (String alias, String task) {
			if (task == null) throw new ArcRuntimeException("import: missing task class name.");
			if (alias == null) {
				Class<?> clazz = null;
				try {
					clazz = ClassReflection.forName(task);
				} catch (ReflectionException e) {
					throw new ArcRuntimeException("import: class not found '" + task + "'");
				}
				alias = clazz.getSimpleName();
			}
			String className = getImport(alias);
			if (className != null) throw new ArcRuntimeException("import: alias '" + alias + "' previously defined already.");
		...
		private ArcRuntimeException stackedTaskException(StackedTask<E> stackedTask, String message) {
			return new ArcRuntimeException(stackedTask.name + " at line " + stackedTask.lineNumber + ": " + message);
		}
