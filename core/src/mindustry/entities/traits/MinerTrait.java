...
public interface MinerTrait extends Entity{
    ...
    /** 可以执行采集资源的距离.<p/>Returns the range at which this miner can mine blocks.*/
    ...
    /** 单位当前采集的瓦砾.<p/>Returns the tile this builder is currently mining. */
    Tile getMineTile();

    /** 设置单位当前采集瓦砾.<p/>Sets the tile this builder is currently mining. */
    void setMineTile(Tile tile);

    /** 获取开采速度.<p/>Returns the mining speed of this miner. 1 = standard, 0.5 = half speed, 2 = double speed, etc. */
    float getMinePower();

    /** 是否可采集指定的物品.<p/>Returns whether or not this builder can mine a specific item type. */
    boolean canMine(Item item);

    /** @return 是否会被Core立即收集开采物品.<p/>whether to offload mined items immediately at the core. if false, items are collected and dropped in a burst. */
