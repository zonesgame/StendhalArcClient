...
import z.entities.type.ai.state.AttackCustomState;
import z.entities.type.ai.state.MachineState;
import z.entities.type.ai.state.SquadState;
 ...
 *  地面单位实体
 * */
public class BaseGroundUnit extends BaseUnit{
    ...
    /** 起始状态*/
    @Deprecated
    public UnitState<? extends BaseGroundUnit> startState;
    /** ai通用数据*/
//    public SquadBlackboard blackboard;

    public UnitState<? extends BaseGroundUnit> squadState, attackCustom, /** 任务简单意识状态*/machineState;

    public BaseGroundUnit() {
    }
...
//    },
    ...
    public UnitState getStartState(){
        return startState == null ? squadState : startState;      // default attack     attackCustom
    }
        ...
        if(!Units.invalidateTarget(target, this)){
            if(dst(target) < getWeapon().bullet.range()){
                ...
                if(Angles.near(angleTo(target), rotation, 13f)){
                    ...
                    Vec2 to = Predict.intercept(BaseGroundUnit.this, target, ammo.speed);

                    getWeapon().update(BaseGroundUnit.this, to.x, to.y);
                }
            }
