...
public class Damage{
    ...
    public static void collideLine(Bullet hitter, Team team, Effect effect, float x, float y, float angle, float length, boolean large){
        ...
        Intc2 collider = (cx, cy) -> {
            ...
            if(tile != null && !collidedBlocks.contains(tile.pos()) && tile.entity != null && tile.getTeamID() != team.id && tile.entity.collide(hitter)){
                ...
                if (enable_isoInput) {
                    hitter.getBulletType().hit(hitter, tile.x, tile.y);
                } else {
    ...
    /** 伤害范围内所有队伍敌人, 坐标系统tile.<p/>Damages all entities and blocks in a radius that are enemies of the team. */
