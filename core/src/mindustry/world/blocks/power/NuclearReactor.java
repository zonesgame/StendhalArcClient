...
public class NuclearReactor extends PowerGenerator{
    ...
    public void update(Tile tile){
        ...
        ConsumeLiquid cliquid = consumes[0].get(ConsumeType.liquid);
        Item item = consumes[0].<ConsumeItems>get(ConsumeType.item).items[0].item;

        int fuel = entity.items.get(item);
        float fullness = (float)fuel / itemCapacity[entity.level()];
    ...
    public void onDestroyed(Tile tile){
        ...
        int fuel = entity.items.get(consumes[0].<ConsumeItems>get(ConsumeType.item).items[0].item);
