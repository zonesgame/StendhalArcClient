...
import mindustry.game.SoundLoop;
...
import z.entities.traits.LevelTrait;
...
public class TileEntity extends BaseEntity implements TargetTrait, HealthTrait, /*zones add*/LevelTrait {
    ...
    /** 绑定Block数据初始化设置.<p/>Sets this tile entity data to this tile, and adds it if necessary. */
...
//        if (block.variantRegions() != null) { // zones add code begon
//            this.variantsIndex = Mathf.random(block.variantRegions().length - 1);
//        } // zones add code end
        return this;
    }
    ...
    public float efficiency(){
        return power != null && (block.consumes[level()].has(ConsumeType.power) && !block.consumes[level()].getPower().buffered) ? power.status : 1f;
    }
