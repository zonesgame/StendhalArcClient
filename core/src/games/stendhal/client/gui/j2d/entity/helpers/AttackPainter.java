...
//    private static final Stroke ARROW_STROKE = new BasicStroke(2);
    ...
    static {
        ...
        ARROW_COLOR.put(Nature.CUT, Color.lightGray);
        ARROW_COLOR.put(Nature.DARK, Color.darkGray);
        ARROW_COLOR.put(Nature.LIGHT, new Color(1, 240 / 255f, 140 / 255f, 1)); // light yellow
        ARROW_COLOR.put(Nature.FIRE, new Color(1, 100 / 255f, 0, 1)); // reddish orange
        ARROW_COLOR.put(Nature.ICE, new Color(140 / 255f, 140 / 255f, 1, 1)); // light blue
    }
...
//                    scaled[i] = new ImageSprite(image);
                    ...
                    int newWidth = orig.getWidth() + (size - 1) * TILE_SIZE;
                    float scaling = ((int) ((newWidth / (float) orig.getWidth()) * 10)) / 10f;

                    final String scaleRef = orig.getReference() + "|" + scaling;
                    Sprite scaleSprite = SpriteStore.get().getScaleCombatSprite(scaleRef);
                    if (scaleSprite == null) {     // 初始化数据
                        scaleSprite = new ImageSprite(((ImageSprite) orig).getImage(), scaleRef).setScale(scaling);
                        SpriteStore.get().putScaleCombatSprite(scaleSprite, scaleRef);
                    }

                    scaled[i] = scaleSprite;
                }
                map.put(d, scaled);
            }
        }
     ...
     */
    public void draw(Graphics g2d, Direction direction, int x, int y,
     ...
     * @param height    entity height
     */
    private void drawAttackSprite(Graphics g, Sprite[] spriteSet, Direction direction,
     ...
     */
    public void drawDistanceAttack(Graphics g2d, RPEntity entity,
