...
public abstract class ClientFramework {
     ...
     * Constructor.
     *
     * @param loggingProperties contains the name of the file that configure the logging
     *                          system.
     */
    ...
    private Proxy discoverProxy(String host, int port) throws IOException {
        ...
        try {
            ...
            IOException ioE = new IOException(translate("Error while discovering proxyserver: %1$s", e.toString()));
            ioE.initCause(e);
            throw ioE;
        }
     ...
     * @throws TimeoutException        if there is no message available in TIMEOUT milliseconds.
     ...
     * @param username Player username
     * @param password Player password
     * @param seed     preauthentication seed
     * @throws InvalidVersionException if we are not using a compatible version
     * @throws TimeoutException        if timeout happens while waiting for the message.
     * @throws LoginFailedException    if login is rejected
     * @throws BannedAddressException
        ...
        while (received < 3) {
            ...
            switch (msg.getType()) {
                ...
                case S2C_LOGIN_SENDNONCE: {
                    ...
                    if (b2 == null) {
                        throw new LoginFailedException(translate("Incorrect hash b2"));
                    }
                    ...
                    if (seed != null) {
                        ...
                        if (bs.length != 16) {
                            throw new LoginFailedException(translate("Seed has not the correct length."));
                        }
                        byte[] b3 = null;
                        b3 = Hash.xor(b1, bs);
                        if (b3 == null) {
                            throw new LoginFailedException(translate("Incorrect hash seed"));
                        }
     ...
     * Login to server using the given username and password.
     *
     * @param username  player username (may be null)
     * @param tokenType type of token (may be null)
     ...
     * @param token     authentication token (usually obtained from 3rd party).
     * @return AccountResult
     * @throws InvalidVersionException if we are not using a compatible version
     * @throws TimeoutException        if timeout happens while waiting for the message.
     * @throws BannedAddressException
     ...
     *
     * @param items in this list by default all items.ack attributes are set to false;
