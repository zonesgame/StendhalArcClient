...
import mindustry.entities.traits.KillerTrait;
...
import static z.debug.ZDebug.enable_drawBulletCollision;
...
public class Bullet extends SolidEntity implements DamageTrait, Scaled, Poolable, DrawTrait, VelocityTrait, TimeTrait, TeamTrait, AbsorbTrait{
    ...
    private Object data;
    /***/
    private boolean supressCollision, supressOnce, /** 初始化*/initialized, /** 偏移*/deflected;
    ...
    public void update(){
        ...
        if(type.hitTiles && collidesTiles() && !supressCollision && initialized){
            if (enable_isoInput) {
                world.raycastEach(Math.round(lastPosition().x), Math.round(lastPosition().y), Math.round(x), Math.round(y), (x, y) -> {
    ...
    public void draw(){
        ...
        float y = this.y;
        if (enable_isoInput) {
            Vec2 pos = Tmp.v11.set(ISOUtils.tileToWorldCoords(this));
            x = pos.x;
            y = pos.y;
        }
        // zones add end
