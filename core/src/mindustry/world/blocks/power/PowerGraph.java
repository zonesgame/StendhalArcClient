...
public class PowerGraph{
    ...
    public float getPowerNeeded(){
        ...
        for(Tile consumer : consumers){
            Consumers consumes = consumer.block().consumes[0];
    ...
    public float getBatteryStored(){
        ...
        for(Tile battery : batteries){
            Consumers consumes = battery.block().consumes[0];
            if(consumes.hasPower()){
    ...
    public float getBatteryCapacity(){
        ...
        for(Tile battery : batteries){
            if(battery.block().consumes[0].hasPower()){
                ConsumePower power = battery.block().consumes[0].getPower();
    ...
    public float getTotalBatteryCapacity(){
        ...
        for(Tile battery : batteries){
            if(battery.block().consumes[0].hasPower()){
                totalCapacity += battery.block().consumes[0].getPower().capacity;
            }
        }
        return totalCapacity;
    }
    ...
    public void add(Tile tile){
        ...
        if(tile.block().outputsPower && tile.block().consumesPower && !tile.block().consumes[0].getPower().buffered){
            producers.add(tile);
            consumers.add(tile);
    ...
    private boolean otherConsumersAreValid(Tile tile, Consume consumePower){
        for(Consume cons : tile.block().consumes[0].all()){
