...
public class Effects{
    ...
    public static void effect(Effect effect, Position loc){
        if (enable_isoInput) {
            Vec2 pos = ISOUtils.tileToWorldCoords(loc);
            provider.createEffect(effect, Color.white, pos.getX(), pos.getY(), 0f, null);
            return;
        }
    ...
    public static void effect(Effect effect, float x, float y, float rotation, Object data){
        ...
        if (enable_isoInput) {
            Vec2 pos = Tmp.v11.set(ISOUtils.tileToWorldCoords(x, y));
            x = pos.x;
            y = pos.y;
    ...
    public static void shake(float intensity, float duration, Position loc){
        if (enable_isoInput) {
            Vec2 pos = ISOUtils.tileToWorldCoords(loc);
            shake(intensity, duration, pos.getX(), pos.getY());
            return;
        }
