...
public class MapInfoDialog extends FloatingDialog{
    ...
    private void setup(){
        ...
        cont.pane(t -> {
            ...
            t.row();
            {   //  添加脚本文件输入框    Core.bundle.format(Strs.str22, entity.level() + 1)
                t.add(Strs.str28).padRight(8).left();
                t.defaults().padTop(15);

                TextField nameScript = t.addField(tags.get("scripts", ""), text -> {
                    tags.put("scripts", text);
                }).size(400, 55f).get();
                nameScript.setMessageText("$unknown");
