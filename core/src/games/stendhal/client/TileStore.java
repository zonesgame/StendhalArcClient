...
class TileStore implements Tileset {
     ...
     */
    private static final String baseFolder = Debug.CONTINUE_TRADING ? getResourceBase() : "data/";
...
//        final URL url = DataLoader.getResource(baseFolder + "tileset/animation.seq");
        final Fi urlHandle = ReplaceFile.getInstance().getFileHandle(baseFolder + "tileset/animation.seq");

//        if (url != null) {
        if (urlHandle.exists()) {
            try {
//                final InputStream in = url.openStream();
                final InputStream in = urlHandle.read();
