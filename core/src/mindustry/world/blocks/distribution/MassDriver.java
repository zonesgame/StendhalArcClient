...
public class MassDriver extends Block{
    ...
    public void update(Tile tile){
        ...
        if(entity.state == DriverState.idle){
            ...
            if(!entity.waitingShooters.isEmpty() && (itemCapacity[entity.level()] - entity.items.total() >= minDistribute)){
        ...
        if(entity.state == DriverState.accepting){
            ...
            if(entity.currentShooter() == null || (itemCapacity[entity.level()] - entity.items.total() < minDistribute)){
            ...
            if(!hasLink || (!entity.waitingShooters.isEmpty() && (itemCapacity[entity.level()] - entity.items.total() >= minDistribute))){
                entity.state = DriverState.idle;
                return;
            }
            ...
            if(
                ...
                link.block().itemCapacity[link.entity.level()] - link.entity.items.total() >= minDistribute //must have minimum amount of space
            ){
    ...
    public boolean acceptItem(Item item, Tile tile, Tile source){
        ...
        return tile.entity.items.total() < itemCapacity[tile.entity.level()] && linkValid(tile);
    }
    ...
    protected void fire(Tile tile, Tile target){
        ...
        for(int i = 0; i < content.items().size; i++){
            int maxTransfer = Math.min(entity.items.get(content.item(i)), ((MassDriver)tile.block()).itemCapacity[entity.level()] - totalUsed);
    ...
    protected void handlePayload(MassDriverEntity entity, Bullet bullet, DriverBulletData data){
        ...
        for(int i = 0; i < data.items.length; i++){
            int maxAdd = Math.min(data.items[i], itemCapacity[entity.level()] * 2 - totalItems);
            entity.items.add(content.item(i), maxAdd);
            data.items[i] -= maxAdd;
            totalItems += maxAdd;

            if(totalItems >= itemCapacity[entity.level()] * 2){
                break;
            }
        }
