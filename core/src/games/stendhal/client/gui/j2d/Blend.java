...
import temp.java.awt.CompositeContext;
...
//    }
    ...
    public String toString() {
        ...
        if (name == null) {
            String colorName = "";
            if (color != null) {
                colorName = "(" + Integer.toHexString(color.toIntBits()) + ")";        // default color.getRGB()
            }
            name = mode + colorName;
        }
        return name;
    ...
    private static class BlendContext implements CompositeContext {
        ...
        BlendContext(Mode mode, Color color) {
            switch (mode) {
                case BLEACH:
                    composer = new BleachComposer(color);
                    break;
                case GENERIC_LIGHT:
                    composer = new GenericLightComposer();
                    break;
                case SOFT_LIGHT:
                    composer = new SoftLightComposer();
                    break;
                case SCREEN:
                    composer = new ScreenComposer();
                    break;
                case TRUE_COLOR:
                    composer = new TrueColorComposer();
                    break;
...
//        }
    ...
    private static class BleachComposer implements Composer {
        ...
        BleachComposer(Color c) {
            int color = c.toIntBits();         // default c.getRGB()
