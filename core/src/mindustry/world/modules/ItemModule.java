...
public class ItemModule extends BlockModule{
    ...
    // zones add begon
    public boolean has(Array<ItemStack> stacks){
        for(ItemStack stack : stacks){
            if(!has(stack.item, stack.amount)) return false;
        }
        return true;
    ...
    public void add(Item item, int amount){
        ...
        if ( itemsListeners != null) {
            for (int i = 0; i < itemsListeners.size; i++) {
                itemsListeners.get(i).addTelegraph(item, amount);
            }
        }
    }
    ...
    public void addAll(ItemModule items){
        ...
        if ( itemsListeners != null) {
            for (int i = 0; i < itemsListeners.size; i++) {
                itemsListeners.get(i).addAllTelegraph(items);
            }
        }
    }
    ...
    public void remove(Item item, int amount){
        ...
        if ( itemsListeners != null) {
            for (int i = 0; i < itemsListeners.size; i++) {
                itemsListeners.get(i).removeTelegraph(item, amount);
            }
        }
    }
    ...
    // zones add begon
    private Array<ItemTelegraph> itemsListeners;

    public ItemModule addListener(ItemTelegraph listener) {
        if (itemsListeners == null)
            itemsListeners = new Array<>(4);
        if (listener != null)
            itemsListeners.add(listener);
        return this;
    }
    public void removeListener(ItemTelegraph listener) {
        if (listener != null)
            itemsListeners.remove(listener, true);
    }
