...
public class Build{
    ...
    /** 返回被拆除的块类型, 如果未成功, 则返回null.<p/>Returns block type that was broken, or null if unsuccesful. */
    ...
    /** 指定位置放置一个BuildBlock.<p/>Places a BuildBlock at this location. */
    @Remote(called = Loc.server)
    ...
    /** 指定队伍的Tile是否可以放在这个位置.<p/>Returns whether a tile can be placed at this location by this team. */
    public static boolean validPlace(Team team, int x, int y, Block type, int rotation){
        if(type == null || !type.isVisible() || type.isHidden()){       // 验证Content是否有效或解锁
            return false;
        }

        if(state.rules.bannedBlocks.contains(type) && !(state.rules.waves && team == state.rules.waveTeam)){    // 验证指定Content是否被剔除
            return false;
        }
    ...
    /** 返回这个位置的Tile是否可以由这个队伍拆除.<p/>Returns whether the tile at this position is breakable by this team */
