...
import static z.debug.ZDebug.enable_drawAllEntity;
...
public class EntityGroup<T extends Entity> implements Iterable<T>{
    ...
    public void draw(Boolf<T> toDraw, Cons<T> cons){
        ...
        for(Entity e : all()){
            ...
            if(enable_drawAllEntity || viewport.overlaps(draw.getX() - draw.drawSize()/2f, draw.getY() - draw.drawSize()/2f, draw.drawSize(), draw.drawSize())){
                cons.get((T)e);
            }
        }
    }
