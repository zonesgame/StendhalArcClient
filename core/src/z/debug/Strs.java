package z.debug;

import arc.Core;

/**
 *
 */
public class Strs {
    public static String get(String key, Object... objects) {
        return Core.bundle.formatValue(key, objects);
    }

    public static String get(String key) {
        return key;
    }

    public static final String str1 = "移动";
    public static final String str2 = "升级";
    public static final String str3 = "[GREEN]升级中...[]";
    public static final String str4 = "[YELLOW]等待中...[]";
    public static final String str5 = "秒";
    public static final String str6 = "分";
    public static final String str7 = "时";
    public static final String str8 = "立刻完成";
    public static final String str9 = "配置";
    public static final String str10 = "使用弹药";
    public static final String str11 = "接收弹药";
    public static final String str12 = "统计";
    public static final String str13 = "接收配置";
    public static final String str14 = "接收";
    public static final String str15 = "获取";
    public static final String str16 = "清空";
    public static final String str17 = "全部";
    public static final String str18 = "工作";
    public static final String str19 = "休息";
    public static final String str20 = "缺少农民";
    public static final String str21 = "生命值 : {0}";
    public static final String str22 = "等级 : {0}";
    public static final String str23 = "状态 : {0}";
    public static final String str24 = "工作中";
    public static final String str25 = "休息中";
    public static final String str26 = "弹药 : {0}";
    public static final String str27 = "Max";
    public static final String str28 = "脚本文件";

    public static final String str29 = "服务器地址";
    public static final String str30 = "服务器端口";
    public static final String str31 = "用户名";
    public static final String str32 = "密码";
    public static final String str33 = "登录服务器";
    public static final String str34 = "保存密码";
    public static final String str35 = "Choose Character";
    public static final String str36 = "New Character";
    public static final String str37 = "Login to {0}";
    public static final String str38 = "Create an account";
    public static final String str39 = "Help";
    public static final String str40 = "Credits";
    public static final String str41 = "Quit";
    public static final String str42 = "[ROYAL]Android zones[] - source code [SKY]https://github.com/zonesgame/StendhalArcClient.git[]";
    public static final String str43 = "";
    public static final String str44 = "";
    public static final String str45 = "";
    public static final String str46 = "";
    public static final String str47 = "";
    public static final String str48 = "";
    public static final String str49 = "";
}
