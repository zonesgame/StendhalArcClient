...
import static z.debug.ZDebug.disable_spawnWarn;
...
public class WaveSpawner{
    ...
    // zones add begon
    /** 塔防模式单位的移动目标点*/
    public Array<Tile> rallyPoints = new Array<>();
    /** 任务模式Player起始点, 重生点. */
    public Array<Tile> playerSpawns = new Array<>();
    // zones add end
    ...
    public void spawnEnemies(){
        ...
        for(SpawnGroup group : state.rules.spawns){
            ...
            if(group.type.flying){
                float spread = enable_isoInput ? margin / 1.5f / tilesize: margin / 1.5f;
                ...
                float spread = enable_isoInput ? tileunit * 2 : tilesize * 2;
    ...
    private void eachGroundSpawn(SpawnConsumer cons){
        for(Tile spawn : groundSpawns){
            if (enable_isoInput)
                cons.accept(spawn.getX(), spawn.getY(), !disable_spawnWarn);
            else
    ...
    private void reset(){
        ...
        for(int x = 0; x < world.width(); x++){
            for(int y = 0; y < world.height(); y++){
                ...
                // zones add begon
                else if (world.tile(x, y).overlay() == Blocks.spawnPlayer) {        // player 重生点
                    playerSpawns.add(world.tile(x, y));
                }
                else if (world.tile(x, y).overlay() == Blocks.rally) {
                    rallyPoints.add(world.tile(x, y));
                }
                // zones add end
            }
        }
