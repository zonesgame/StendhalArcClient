...
package arc.ai.pfa.indexed;

import arc.ai.pfa.Connection;
import arc.ai.pfa.GraphPath;
import arc.ai.pfa.Heuristic;
import arc.ai.pfa.PathFinder;
import arc.ai.pfa.PathFinderQueue;
import arc.ai.pfa.PathFinderRequest;
import arc.func.Boolf2;
import arc.struct.Array;
import arc.struct.BinaryHeap;
...
public class IndexedAStarPathFinder<N> implements PathFinder<N> {
	...
	// zones add begon
	public boolean searchNodePathNoLast (N startNode, N endNode, Heuristic<N> heuristic, GraphPath<N> outPath, Boolf2 checkOver) {

		// Perform AStar
		boolean found = searchNoLastNode(startNode, endNode, heuristic, checkOver);
	...
	protected boolean searchNoLastNode (N startNode, N endNode, Heuristic<N> heuristic, Boolf2 checkOver) {
...
//				if (current.node.equals(endNode)) return true;
				// 权倾天下数据
				if (checkOver.get(current.node, endNode)) return true;
			}
...
//	private Boolf2<N, N> checkOver = null;
	// zones add end
}
