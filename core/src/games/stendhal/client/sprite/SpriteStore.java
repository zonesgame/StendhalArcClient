...
//        }
     ...
     *
     * @param sprite     The image which contains the different frames.
     ...
     *
     * @param icon Name of pixmaps without full path
     * @return A sprite instance containing an accelerate
    ...
    public void putScaleCombatSprite(final Sprite sprite, final String parentRef) {
        if (sprite == null) return;

//        String ref = parentRef + '|' + scale;
        final SpriteCache cache = SpriteCache.get();
        cache.add(parentRef, sprite);
    }

    @ZonesAnnotate.ZMethod
    public Sprite getScaleCombatSprite(Object parentRef) {
        final SpriteCache cache = SpriteCache.get();
//        String ref = parentRef + "|" + scale;
        return cache.get(parentRef);
    }
...
//        }
     ...
     * @return base sprite colored with color
     */
    public Sprite getModifiedSprite(final String baseRef, final Color color, final Composite blend) {
...
//        return sprite;
    ...
    public String createModifiedRef(String baseRef, Color color, Composite blend) {
        String colorName = Integer.toHexString(color.toIntBits());     // color.getRGB()
...
//        return url != null;
        final Fi urlHandle = ReplaceFile.getInstance().getFileHandle(ref);
        return urlHandle.exists();
    }
...
//        return sprite;
        ...
        // No blood mode
//        boolean showBlood = WtWindowManager.getInstance().getPropertyBoolean("gamescreen.blood", true);
        boolean showBlood = true;
        String safeRef = ref.split(".png")[0] + "-safe.png";
        Fi safeHandle = ReplaceFile.getInstance().getFileHandle(safeRef);
        if (!showBlood && (safeHandle.exists())) {
            logger.debug("Using safe image: " + safeRef);
            ref = safeRef;
        }
