...
public class ClientGameConfiguration {
	...
	private Properties getGameProperties() {
		try {
			InputStream is = ReplaceFile.getInstance().getResourceAsStream(ClientGameConfiguration.class, "game.properties");
			if (is == null) {
				is = ReplaceFile.getInstance().getResourceAsStream(ClientGameConfiguration.class, "game-default.properties");
			}
