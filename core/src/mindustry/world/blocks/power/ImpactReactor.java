...
public class ImpactReactor extends PowerGenerator{
    ...
    public void setBars(){
        ...
        Strings.fixed(Math.max(entity.block.getPowerProduction(entity.tile) - consumes[0].getPower().usage, 0) * 60 * entity.timeScale, 1)),
        () -> Pal.powerBar,
    ...
    public void update(Tile tile){
        ...
        if(entity.cons.valid() && entity.power.status >= 0.99f){
            boolean prevOut = getPowerProduction(tile) <= consumes[0].getPower().requestedPower(entity);
            ...
            if(!prevOut && (getPowerProduction(tile) > consumes[0].getPower().requestedPower(entity))){
