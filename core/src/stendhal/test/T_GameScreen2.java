...
import temp.Debug;
import temp.java.awt.Dimension;
 ...
 */
public final class T_GameScreen2 implements IGameScreen, DropTarget,
        GameObjects.GameObjectListener, StendhalClient.ZoneChangeListener, PositionChangeListener {

    /**serial version uid.*/
    private static final long serialVersionUID = -4070406295913030925L;
    private static final Logger logger = Logger.getLogger(T_GameScreen2.class);
...
//        keyEventMapping.put(KeyEvent.VK_0, Integer.valueOf(10));
    ...
    /**Speed factor for centering the screen. Smaller is faster,<p/> and keeps the player closer to the center of the screen when walking.*/
    private static final int PAN_INERTIA = 15;
    /**Space at the right and bottom of the screen next to the off line indicator icon.*/
    private static final int OFFLINE_MARGIN = 10;
    private static final Sprite offlineIcon;

    /** the singleton instance. */
    private static T_GameScreen2 screen;
...
//    private final GroundContainer ground;
    ...
    /**Flag for telling if the screen should be scaled if it's not of the default size.*/
    private boolean useScaling = true;

    /**A flag for telling if the screen is being actually scaled. Used for detecting if the ground layers will need triple buffering.*/
    private boolean useTripleBuffer;
    /**Buffer for drawing the ground layers when the screen is scaled.*/
    private Object buffer;           //  VolatileImage buffer;
     ...
     */
    public T_GameScreen2(final StendhalClient client) {
...
//        setIgnoreRepaint(true);
     ...
     * if screen scaling is used. This is for adjusting the internal parameters for the change.
     */
     ...
     * 	will be scaled to fit the screen size, otherwise it will be drawn using the native resolution.
     */
     ...
     * @return <code>true</code> if the graphics are scaled to the screen size,<code>false</code> if the native resolution is used
     */
     ...
     */
    public static void setDefaultScreen(T_GameScreen2 screen) {
        T_GameScreen2.screen = screen;
    }

    /** @return the GameScreen object. */
    public static T_GameScreen2 get() {
        return screen;
    }

    /** @return screen width in world units.(tile) */
    private int getViewWidth() {
        return (int) Math.ceil(sw / (SIZE_UNIT_PIXELS * scale));
    }

    /** @return screen height in world units .(tile)*/
    ...
    public void addEntity(final IEntity entity) {
        ...
        if (view != null) {
            if (view instanceof Entity2DView) {
                ...
                if (Debug.NOTE1)
                    ;
                inspectable.setInspector(null);
//                inspectable.setInspector(ground);
            }
            if (entity.isUser()) {
                Core.app.post(this::center);
//                SwingUtilities.invokeLater(this::center);
    ...
    public void addEffect(final EffectLayer effect) {
        Core.app.post(() -> globalEffects.add(effect));
//        SwingUtilities.invokeLater(() -> globalEffects.add(effect));
    ...
    /** Calculate the target speed for moving the view position. The farther away, the faster.*/
...
//            camy = y * SIZE_UNIT_PIXELS;
//            camera.position.set(camx, camy);
//            camera.position.set(ix * SIZE_UNIT_PIXELS, iy * SIZE_UNIT_PIXELS);
//            camera.update();
...
//        graphics.dispose();
     ...
     * @param fullRedraw <code>true</code> if this is called for a full game screen redraw
     */
...
//        Rectangle clip = g.getClipBounds();
        ...
        {
//            startTileX = Math.max(0, (int) getViewX() - 1);
//            startTileY = Math.max(0, (int) getViewY() - 1);
        }
...
//            System.out.println(getViewX() + "  " + getViewY() + "  " + getViewWidth() + "  " + getViewHeight());
    ...
    /**
     * Convert a world x coordinate to <em>raw</em> (native resolution) screen x coordinate.
    ...
    /**
     * Convert a world y coordinate to <em>raw</em> (native resolution) screen y coordinate.
...
////            float x = svx  + sw / 2;
////            float y = svy  + sh / 2;
////            y = (float)((StendhalClient.get().getStaticGameLayers().getHeight() - y - 1) * 32f);
////            camera.position.set(x, y);
////            camera.update();
////            camera.position.lerpDelta(new Vec2((float)x * 32f, (float)((StendhalClient.get().getStaticGameLayers().getHeight() - y - 1) * 32f)), 0.8f);
//
//            float camx = ix * SIZE_UNIT_PIXELS;
//            float camy = (float) ((StendhalClient.get().getStaticGameLayers().getHeight() - iy - 1) * SIZE_UNIT_PIXELS);
//            camera.position.set(camx, camy);
////            camera.position.set(ix * SIZE_UNIT_PIXELS, iy * SIZE_UNIT_PIXELS);
//            camera.update();
//            System.out.println(camera.position.x + " X " + camera.position.y + " # " + ix + " x " + iy + "   " + (StendhalClient.get().getStaticGameLayers().getHeight()));
//            System.out.println(StendhalClient.get().getStaticGameLayers().getHeight() + "  " +
//                    sh + "  " + this.wh + "  " + this.getScreenViewY() + " " + this.gameLayers.getHeight());
        }
    }
...
//        ground.dropEntity(entity, amount, point);
     ...
     * @param sprite sprite
     * @param persistTime time to stay on the screen before being automatically removed
     *  @param priority importance of the message to keep it above others
     */
    ...
    public void onZoneChange(Zone zone) {
        removeAllObjects();
        Core.app.post(globalEffects::clear);
