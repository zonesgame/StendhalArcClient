...
public class LaunchPad extends StorageBlock{
    ...
    public boolean acceptItem(Item item, Tile tile, Tile source){
        return item.type == ItemType.material && tile.entity.items.total() < itemCapacity[tile.entity.level()];
    }
    ...
    public void draw(Tile tile){
        ...
        float progress = Mathf.clamp(Mathf.clamp((tile.entity.items.total() / (float)itemCapacity[tile.entity.level()])) * ((tile.entity.timer.getTime(timerLaunch) / (launchTime / tile.entity.timeScale))));
        float scale = size / 3f;
    ...
    public void update(Tile tile){
        ...
        if(world.isZone() && entity.cons.valid() && entity.items.total() >= itemCapacity[entity.level()] && entity.timer.get(timerLaunch, launchTime / entity.timeScale)){
            for(Item item : Vars.content.items()){
                Events.fire(Trigger.itemLaunch);
                Effects.effect(Fx.padlaunch, tile);
                int used = Math.min(entity.items.get(item), itemCapacity[entity.level()]);
